...
class Tree {
  ...
  addOne(device, prop, val, emptyParent) {
    let updated;
    let added;
    let ext;
    ...
    if (!dnode) {
      ...
      if (emptyParent) return { added };
    } else {
      if (dnode.title !== device.title) {
        dnode.title = device.title;
        ext = { id: dnode.id, title: dnode.title }
      }
    }
