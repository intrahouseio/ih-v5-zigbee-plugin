...
class Scanner {
  ...
  process(device, prop, val, emptyParent) {
    this.plugin.log('Scan process '+prop+'='+val+' device: '+util.inspect(device))
    let result = this.scanTree.addOne(device, prop, val, emptyParent);
    ...
    if (result && this.status == 2) {
      ...
      if(result.ext) {
        this.plugin.send({ type: 'scan', op: 'update', data: { [result.ext.id]: result.ext}, scanid: 'root' });
      } 
