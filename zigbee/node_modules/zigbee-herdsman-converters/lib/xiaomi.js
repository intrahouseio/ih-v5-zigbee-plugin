...
exports.fromZigbee = exports.modernExtend = exports.xiaomiModernExtend = exports.manufacturerCode = exports.trv = exports.fp1 = exports.numericAttributes2Options = exports.VOCKQJK11LMDisplayUnit = exports.numericAttributes2Payload = exports.buffer2DataObject = void 0;
...
const buffer2DataObject = (meta, model, buffer) => {
        ...
        for (let i = 0; i < buffer.length - 1; i++) {
            ...
            switch (buffer[i + 1]) {
                ...
                case 66:
                    ...
                    if (meta.logger)
                        meta.logger.debug(`${model.model}: unknown vtype=${buffer[i + 1]}, pos=${i + 1}, moving length 1`);
                    i += 2;
                    break;
                case 95:
                    // 0x5f unknown, length taken from what seems correct in the logs, maybe is wrong
                    if (meta.logger)
                        meta.logger.debug(`${model.model}: unknown vtype=${buffer[i + 1]}, pos=${i + 1}, moving length 4`);
                    i += 5;
                    break;
                default:
                    if (meta.logger)
                        meta.logger.debug(`${model.model}: unknown vtype=${buffer[i + 1]}, pos=${i + 1}`);
            }
            if (value != null) {
    ...
    if (meta.logger) {
        meta.logger.debug(`${model.model}: Processed buffer into data ${JSON.stringify(dataObject, (key, value) => typeof value === 'bigint' ? value.toString() : value)}`);
    }
    return dataObject;
};
        ...
        switch (key) {
            ...
            case '3':
                if (['WXCJKG11LM', 'WXCJKG12LM', 'WXCJKG13LM', 'MCCGQ14LM', 'GZCGQ01LM', 'JY-GZ-01AQ', 'CTP-R01'].includes(model.model)) {
                    ...
                    (0, utils_1.assertNumber)(value);
                    payload.device_temperature = (0, utils_1.calibrateAndPrecisionRoundOptions)(value, options, 'device_temperature'); // 0x03
                }
                break;
            case '4':
            ...
            case '11':
                if (['RTCGQ11LM'].includes(model.model)) {
                    (0, utils_1.assertNumber)(value);
                    payload.illuminance = (0, utils_1.calibrateAndPrecisionRoundOptions)(value, options, 'illuminance');
                    // DEPRECATED: remove illuminance_lux here.
                    payload.illuminance_lux = (0, utils_1.calibrateAndPrecisionRoundOptions)(value, options, 'illuminance_lux');
                }
                break;
            case '12':
                if (['ZNLDP13LM', 'ZNXDD01LM'].includes(model.model)) {
            ...
            case '100':
                if (['QBKG20LM', 'QBKG31LM', 'QBKG39LM', 'QBKG41LM', 'QBCZ15LM', 'LLKZMK11LM', 'QBKG12LM', 'QBKG03LM', 'QBKG25LM']
                    ...
                    if (temperature > -65 && temperature < 65) {
                        payload.temperature = (0, utils_1.calibrateAndPrecisionRoundOptions)(temperature, options, 'temperature');
                    }
                }
            ...
            case '101':
                if (['QBKG20LM', 'QBKG31LM', 'QBKG39LM', 'QBKG41LM', 'QBCZ15LM', 'QBKG25LM', 'QBKG34LM', 'LLKZMK11LM', 'QBKG12LM', 'QBKG03LM']
                    ...
                    if (humidity >= 0 && humidity <= 100) {
                        payload.humidity = (0, utils_1.calibrateAndPrecisionRoundOptions)(humidity, options, 'humidity');
                    }
                }
            ...
            case '102':
                if (['QBKG25LM', 'QBKG34LM'].includes(model.model)) {
                    ...
                    (0, utils_1.assertNumber)(value);
                    payload.pressure = (0, utils_1.calibrateAndPrecisionRoundOptions)(value / 100.0, options, 'pressure');
                }
            ...
            case '149':
                (0, utils_1.assertNumber)(value);
                payload.energy = (0, utils_1.calibrateAndPrecisionRoundOptions)(value, options, 'energy'); // 0x95
            ...
            case '150':
                if (!['JTYJ-GD-01LM/BW'].includes(model.model)) {
                    (0, utils_1.assertNumber)(value);
                    payload.voltage = (0, utils_1.calibrateAndPrecisionRoundOptions)(value * 0.1, options, 'voltage'); // 0x96
                }
                break;
            case '151':
                if (['LLKZMK11LM'].includes(model.model)) {
                    (0, utils_1.assertNumber)(value);
                    payload.current = (0, utils_1.calibrateAndPrecisionRoundOptions)(value, options, 'current');
                }
                else {
                    (0, utils_1.assertNumber)(value);
                    payload.current = (0, utils_1.calibrateAndPrecisionRoundOptions)(value * 0.001, options, 'current');
                }
                break;
            case '152':
                if (['DJT11LM'].includes(model.model)) {
                    ...
                    (0, utils_1.assertNumber)(value);
                    payload.power = (0, utils_1.calibrateAndPrecisionRoundOptions)(value, options, 'power'); // 0x98
                }
                break;
            case '154':
                if (['ZNLDP13LM', 'ZNXDD01LM'].includes(model.model)) {
            ...
            case '276':
                if (['VOCKQJK11LM'].includes(model.model)) {
                    payload.display_unit = (0, utils_1.getKey)(exports.VOCKQJK11LMDisplayUnit, value);
                }
                break;
            case '293':
                payload.click_mode = (0, utils_1.getFromLookup)(value, { 1: 'fast', 2: 'multi' });
            ...
            case '320':
                if (['MCCGQ13LM'].includes(model.model)) {
                    payload.battery_cover = (0, utils_1.getFromLookup)(value, { 0: 'CLOSE', 1: 'OPEN' });
                }
                break;
            case '322':
                if (['RTCZCGQ11LM'].includes(model.model)) {
            ...
            default:
                if (meta.logger)
                    meta.logger.debug(`${model.model}: unknown key ${key} with value ${value}`);
        }
    }
    if (meta.logger)
        meta.logger.debug(`${model.model}: Processed data into payload ${JSON.stringify(payload)}`);
    return payload;
};
exports.numericAttributes2Payload = numericAttributes2Payload;
exports.VOCKQJK11LMDisplayUnit = {
...
const numericAttributes2Options = (definition) => {
    ...
    for (const expose of definition.exposes) {
        ...
        if (supported.includes(expose.name) && !isBatteryVoltage) {
            const type = (0, utils_1.calibrateAndPrecisionRoundOptionsIsPercentual)(expose.name) ? 'percentual' : 'absolute';
...
exports.fp1 = {
    ...
    encodeXCellsDefinition: (xCells) => {
        ...
        return [...xCells.values()].reduce((accumulator, marker) => accumulator + exports.fp1.encodeXCellIdx(marker), 0);
    },
    /**
    ...
    parseAqaraFp1RegionDeleteInput: (input) => {
        if (!input || typeof input !== 'object') {
            return exports.fp1.failure({ reason: 'NOT_OBJECT' });
        }
        if (!('region_id' in input) || !exports.fp1.isAqaraFp1RegionId(input.region_id)) {
            return exports.fp1.failure({ reason: 'INVALID_REGION_ID' });
        }
        return {
            /** @type true */
            isSuccess: true,
    ...
    parseAqaraFp1RegionUpsertInput: (input) => {
        ...
        if (!('zones' in input) || !Array.isArray(input.zones) || !input.zones.length) {
            return exports.fp1.failure({ reason: 'ZONES_LIST_EMPTY' });
        }
        if (!input.zones.every(exports.fp1.isAqaraFp1RegionZoneDefinition)) {
            return exports.fp1.failure({ reason: 'INVALID_ZONES' });
        }
        return {
            /** @type true */
            isSuccess: true,
    ...
    isAqaraFp1RegionId: (value) => {
        return (typeof value === 'number' &&
            value >= exports.fp1.constants.region_config_regionId_min &&
            value <= exports.fp1.constants.region_config_regionId_max);
    },
    // Note: this is valid typescript JSDoc
    // eslint-disable-next-line valid-jsdoc
    /**
    ...
    isAqaraFp1RegionZoneDefinition: (value) => {
        return (value &&
            ...
            value.x >= exports.fp1.constants.region_config_zoneX_min &&
            value.x <= exports.fp1.constants.region_config_zoneX_max &&
            value.y >= exports.fp1.constants.region_config_zoneY_min &&
            value.y <= exports.fp1.constants.region_config_zoneY_max);
    },
    /**
...
exports.trv = {
    ...
    decodeHeartbeat(meta, model, messageBuffer) {
        const data = (0, exports.buffer2DataObject)(meta, model, messageBuffer);
        const payload = {};
...
exports.xiaomiModernExtend = {
    xiaomiSwitchType: (args) => modernExtend.enumLookup({
        name: 'switch_type',
    ...
    xiaomiPowerOnBehavior: (args) => modernExtend.enumLookup({
    ...
    xiaomiOperationMode: (args) => modernExtend.enumLookup({
        name: 'operation_mode',
    ...
    xiaomiAction: (args) => modernExtend.actionEnumLookup({
        lookup: { 'single': 1 },
...
exports.modernExtend = exports.xiaomiModernExtend;
exports.fromZigbee = {
    xiaomi_basic: {
        cluster: 'genBasic',
        type: ['attributeReport', 'readResponse'],
        options: exports.numericAttributes2Options,
    ...
    xiaomi_basic_raw: {
        ...
        convert: async (model, msg, publish, options, meta) => {
            ...
            if (Buffer.isBuffer(msg.data)) {
                const dataObject = (0, exports.buffer2DataObject)(meta, model, msg.data);
...
exports.numericAttributes2Options = exports.numericAttributes2Options;
exports.VOCKQJK11LMDisplayUnit = exports.VOCKQJK11LMDisplayUnit;
exports.fp1 = exports.fp1;
exports.trv = exports.trv;
