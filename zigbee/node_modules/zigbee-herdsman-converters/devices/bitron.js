...
const manufacturerOptions = { manufacturerCode: zigbee_herdsman_1.Zcl.ManufacturerCode._4_NOKS };
const bitron = {
    fz: {
        thermostat_hysteresis: {
            ...
            convert: (model, msg, publish, options, meta) => {
                ...
                if (msg.data.hasOwnProperty('fourNoksHysteresisHigh')) {
                    if (!result.hasOwnProperty('hysteresis'))
                        result.hysteresis = {};
...
const definitions = [
    ...
    },
    {
        ...
        configure: async (device, coordinatorEndpoint) => {
            const endpoint = device.getEndpoint(1);
            await reporting_1.default.bind(endpoint, coordinatorEndpoint, ['genPowerCfg', 'genBasic', 'genOnOff', 'genLevelCtrl']);
        },
    },
    {
    ...
    },
    {
        ...
        configure: async (device, coordinatorEndpoint, logger) => {
            const endpoint = device.getEndpoint(1);
            await reporting_1.default.bind(endpoint, coordinatorEndpoint, ['genOnOff', 'seMetering']);
            await reporting_1.default.instantaneousDemand(endpoint);
            await reporting_1.default.currentSummDelivered(endpoint);
            try {
                await reporting_1.default.currentSummReceived(endpoint);
            }
            catch (error) {
    ...
    },
    {
        ...
        toZigbee: [
            toZigbee_1.default.thermostat_control_sequence_of_operation, toZigbee_1.default.thermostat_occupied_heating_setpoint,
            toZigbee_1.default.thermostat_occupied_cooling_setpoint, toZigbee_1.default.thermostat_local_temperature_calibration,
            toZigbee_1.default.thermostat_local_temperature, toZigbee_1.default.thermostat_running_state, toZigbee_1.default.thermostat_temperature_display_mode,
            toZigbee_1.default.thermostat_keypad_lockout, toZigbee_1.default.thermostat_system_mode, toZigbee_1.default.battery_voltage, bitron.tz.thermostat_hysteresis,
        ],
        exposes: (device, options) => {
            var _a;
            const dynExposes = [];
            let ctrlSeqeOfOper = ((_a = device === null || device === void 0 ? void 0 : device.getEndpoint(1).getClusterAttributeValue('hvacThermostat', 'ctrlSeqeOfOper')) !== null && _a !== void 0 ? _a : null);
            const modes = [];
            // NOTE: ctrlSeqeOfOper defaults to 2 for this device (according to the manual)
            if (ctrlSeqeOfOper == null)
                ctrlSeqeOfOper = 2;
        ...
        configure: async (device, coordinatorEndpoint, logger) => {
            ...
            await reporting_1.default.batteryAlarmState(endpoint);
    ...
    },
    {
        ...
        configure: async (device, coordinatorEndpoint, logger) => {
            const endpoint = device.getEndpoint(1);
            await reporting_1.default.bind(endpoint, coordinatorEndpoint, ['genOnOff', 'genScenes']);
        },
    },
    {
