...
class Device extends entity_1.default {
    constructor(ID, type, ieeeAddr, networkAddress, manufacturerID, endpoints, manufacturerName, powerSource, modelID, applicationVersion, stackVersion, zclVersion, hardwareVersion, dateCode, softwareBuildID, interviewCompleted, meta, lastSeen, useImplicitCheckin, defaultSendWhenActive) {
        super();
        this.ID = ID;
        this._type = type;
    ...
    get defaultSendWhenActive() { return this._defaultSendWhenActive; }
    set defaultSendWhenActive(defaultSendWhenActive) {
        this._defaultSendWhenActive = defaultSendWhenActive;
        for (const endpoint of this._endpoints) {
    ...
    createEndpoint(ID) {
        ...
        const endpoint = endpoint_1.default.create(ID, undefined, undefined, [], [], this.networkAddress, this.ieeeAddr, this.defaultSendWhenActive || false);
    ...
    static create(type, ieeeAddr, networkAddress, manufacturerID, manufacturerName, powerSource, modelID, interviewCompleted, endpoints) {
        ...
        const endpointsMapped = endpoints.map((e) => {
            return endpoint_1.default.create(e.ID, e.profileID, e.deviceID, e.inputClusters, e.outputClusters, networkAddress, ieeeAddr, false);
        });
        const ID = entity_1.default.database.newID();
        const device = new Device(ID, type, ieeeAddr, networkAddress, manufacturerID, endpointsMapped, manufacturerName, powerSource, modelID, undefined, undefined, undefined, undefined, undefined, undefined, interviewCompleted, {}, null, true, undefined);
    ...
    async interviewInternal() {
        ...
        if (this.manufacturerID === 4619 && this._type === 'EndDevice') {
            ...
            try {
                const endpoint = endpoint_1.default.create(1, undefined, undefined, [], [], this.networkAddress, this.ieeeAddr, false);
        ...
        activeEndpoints.endpoints.filter((e) => e !== 0 && !this.getEndpoint(e)).forEach((e) => this._endpoints.push(endpoint_1.default.create(e, undefined, undefined, [], [], this.networkAddress, this.ieeeAddr, false)));
        this.save();
    ...
    async removeFromDatabase() {
        ...
        const newEndpoints = [];
        for (const endpoint of this.endpoints) {
            newEndpoints.push(endpoint_1.default.create(endpoint.ID, endpoint.profileID, endpoint.deviceID, endpoint.inputClusters, endpoint.outputClusters, this.networkAddress, this.ieeeAddr, this.defaultSendWhenActive));
        }
