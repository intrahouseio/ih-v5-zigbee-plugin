...
class Controller extends events_1.default.EventEmitter {
    ...
    async backup() {
        if (this.options.backupPath && await this.adapter.supportsBackup()) {
            ...
            fs_1.default.writeFileSync(this.options.backupPath, JSON.stringify(unifiedBackup, null, 2));
    ...
    onNetworkAddress(payload) {
        ...
        this.emit(Events.Events.lastSeenChanged, { device, reason: 'networkAddress' });
    ...
    onDeviceAnnounce(payload) {
        ...
        this.emit(Events.Events.lastSeenChanged, { device, reason: 'deviceAnnounce' });
    ...
    async onDeviceJoined(payload) {
        ...
        device.updateLastSeen();
        this.emit(Events.Events.lastSeenChanged, { device, reason: 'deviceJoined' });
        device.implicitCheckin();
    ...
    async onZclOrRawData(dataType, dataPayload) {
        ...
        if (type && data) {
            ...
            this.emit(Events.Events.lastSeenChanged, { device, reason: 'messageEmitted' });
        }
        else {
            this.emit(Events.Events.lastSeenChanged, { device, reason: 'messageNonEmitted' });
        }
        if (this.isZclDataPayload(dataPayload, dataType)) {
