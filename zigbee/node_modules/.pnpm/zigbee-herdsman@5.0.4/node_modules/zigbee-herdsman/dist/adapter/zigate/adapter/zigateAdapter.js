...
class ZiGateAdapter extends adapter_1.default {
    ...
    async start() {
        ...
        try {
            ...
            await this.driver.sendCommand(constants_1.ZiGateCommandCode.SetDeviceType, {
                deviceType: constants_1.DeviceType.Coordinator,
            });
            await this.initNetwork();
            await this.driver.sendCommand(constants_1.ZiGateCommandCode.AddGroup, {
                addressMode: constants_1.AddressMode.Short,
    ...
    async getCoordinatorVersion() {
        ...
        const meta = {
            transportrev: 0,
            product: 0,
            majorrel: Number.parseInt(result.payload.major).toString(16),
            minorrel: Number.parseInt(result.payload.minor).toString(16),
            maintrel: Number.parseInt(result.payload.revision).toString(16),
            revision: Number.parseInt(result.payload.revision).toString(16),
        };
        return {
            type: "zigate",
            meta: meta,
    ...
    async addInstallCode(_ieeeAddress, _key, _hashed) {
    ...
    async sendZclFrameToEndpointInternal(ieeeAddr, networkAddress, endpoint, sourceEndpoint, zclFrame, timeout, disableResponse, disableRecovery, responseAttempt, dataRequestAttempt, checkedNetworkAddress, discoveredRoute) {
        ...
        const payload = {
            addressMode: constants_1.AddressMode.Short, //nwk
    ...
    async sendZclFrameToGroup(groupID, zclFrame, sourceEndpoint) {
        return await this.queue.execute(async () => {
            const data = zclFrame.toBuffer();
            const payload = {
                addressMode: constants_1.AddressMode.Group, //nwk
    ...
    async initNetwork() {
        ...
        try {
            ...
            logger_1.logger.debug(`Set EPanID ${
            // biome-ignore lint/style/noNonNullAssertion: ignored using `--suppress`
            this.networkOptions.extendedPanID.toString()}`, NS);
    ...
    async sendZclFrameInterPANToIeeeAddr(_zclFrame, _ieeeAddress) {
        await Promise.reject(new Error("Not supported"));
    }
